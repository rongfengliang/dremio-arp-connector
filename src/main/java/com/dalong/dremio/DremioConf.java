...
@SourceType(value = "DREMIO", label = "DREMIO")
public class DremioConf extends AbstractArpConf<DremioConf> {
    private static final String ARP_FILENAME = "arp/implementation/dremio-arp.yaml";
    private static final Logger logger = Logger.getLogger(DremioConf.class);
    private static final ArpDialect ARP_DIALECT = AbstractArpConf.loadArpFile(ARP_FILENAME, DremioDialect::new);
    private static final String DRIVER = "com.dremio.jdbc.Driver";
    static class DremioSchemaFetcher extends ArpDialect.ArpSchemaFetcher {
        private final String query;
        @VisibleForTesting
        public String getQuery() {
            return this.query;
        }
        public DremioSchemaFetcher(String query, JdbcPluginConfig config) {
            super(query, config);
            this.query = query;
        }
        ...
        public JdbcFetcherProto.GetTableMetadataResponse getTableMetadata(JdbcFetcherProto.GetTableMetadataRequest request) {
            ...
            logger.info(String.format("getTableMetadata  name:--------%s",request.toString()));
        ...
        private JdbcFetcherProto.CanonicalizeTablePathResponse getDatasetHandleViaGetTables(JdbcFetcherProto.CanonicalizeTablePathRequest request, Connection connection) throws SQLException {
            ...
            ResultSet tablesResult = metaData.getTables(null, request.getSchema(), null, (String[])null);
            Throwable throwable = null;
            try {
                while(tablesResult.next()) {
                    ...
                    logger.info(String.format("getDatasetHandleViaGetTables schema name:--------%s",tablesResult.getString(2)));
                    logger.info(String.format("schema name  %s", tablesResult.getString(2)));
                    logger.info(String.format("table name  %s", tablesResult.getString(3)));
                    responseBuilder.setSchema(tablesResult.getString(2));
...
//            PreparedStatement statement = connection.prepareStatement("SELECT * FROM app.demoapp");
            ...
            try {
                ...
                com.dremio.exec.store.jdbc.JdbcFetcherProto.CanonicalizeTablePathResponse.Builder responseBuilder = JdbcFetcherProto.CanonicalizeTablePathResponse.newBuilder();
                responseBuilder.setSchema(request.getSchema());
            ...
            public JdbcFetcherProto.CanonicalizeTablePathResponse computeNext() {
                try {
                    if (this.tablesResult != null && this.tablesResult.next()) {
                        ...
                        logger.info(String.format("computeNext query:--------%s",this.tablesResult.getString(2)));
                        logger.info(String.format("computeNext query:--------%s",this.tablesResult.getString(3)));
                        response.setSchema(this.tablesResult.getString(2));
    ...
    static class DremioDialect extends ArpDialect {

        public DremioDialect(ArpYaml yaml) {
            super(yaml);
        }
        ...
        public JdbcSchemaFetcherImpl newSchemaFetcher(JdbcPluginConfig config) {
           // String query = "SELECT '' CAT,  '' SCH, TABLE_NAME NME from information_schema.tables where 1=1  and table_schema='doc'";
            String query = "SELECT '' CAT,  TABLE_SCHEMA SCH, TABLE_NAME NME from information_schema.\"tables\" where 1=1";
            return new DremioSchemaFetcher(query,config);
        }
        @Override
    ...
    public String toJdbcConnectionString() {
       ...
       // jdbc:dremio:direct=<DREMIO_COORDINATOR>:31010[;schema=<OPTIONAL_SCHMEMA>]
        final String format = String.format("jdbc:dremio:direct=%s:%d", this.host, this.port);
        logger.info("=========dremio============"+format);
        return format;
    }
    @Override
    @VisibleForTesting
